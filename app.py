...
def init_db():
    ...
    # Tabela de Novas Requisições de Compras de Ativos
...
def estrutura():
    ...
    itens_estoque = conn.execute('''
        SELECT p.id AS produto_id, p.codigo_produto, p.nome_produto, COALESCE(ep.quantidade_disponivel, 0) AS quantidade_disponivel 
        FROM produtos p LEFT JOIN estoque_produtos ep ON p.id = ep.produto_id
    ''').fetchall()
    
    conn.close()
    return render_template('estrutura.html', taxa_atual=11.39, registros=registros, maquinas=maquinas_dados, materiais=mats, produtos=prods, estoque_itens=itens_estoque)
...
def salvar_estrutura():
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?, ?, ?)
    ''', (
        request.form.get('turma_nome', 'Sem Nome'),
        float(request.form.get('valor_terreno') or 0),
        float(request.form.get('valor_instalacoes') or 0),
        float(request.form.get('taxa_selic') or 0),
        float(request.form.get('aluguel_regional') or 0)
    ))
    conn.commit()
    conn.close()
    return redirect(url_for('estrutura'))
...
def maquinas():
    ...
    custo_min = base / (176 * 60) if base > 0 else 0
    return render_template('maquinas.html', maquinas=m_dados, custo_minuto_estrutural=custo_min, estoque_itens=itens_estoque)
...
def salvar_maquina():
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        request.form.get('nome_equipamento', 'Sem Nome'),
        float(request.form.get('potencia') or 0),
        float(request.form.get('consumo_eletrico') or 0),
        request.form.get('velocidade', ''),
        request.form.get('avanco', ''),
        float(request.form.get('comprimento_max') or 0),
        float(request.form.get('diametro_max') or 0),
        int(request.form.get('frequencia_manutencao') or 0),
        int(request.form.get('horas_trabalhadas') or 0),
        float(request.form.get('preco_compra') or 0),
        float(request.form.get('depreciacao_mensal') or 0),
        float(request.form.get('valor_venda_final') or 0),
        float(request.form.get('custo_minuto_maquina') or 0)
    ))
    conn.commit()
    conn.close()
...
def alterar_maquina(id):
    ...
    conn.execute('''
        UPDATE maquinas 
        SET nome_equipamento=?, potencia=?, consumo_eletrico=?, velocidade=?, avanco=?, comprimento_max=?, diametro_max=?, frequencia_manutencao=?, horas_trabalhadas=?, preco_compra=?, depreciacao_mensal=?, valor_venda_final=?, custo_minuto_maquina=? 
        WHERE id=?
    ''', (
        request.form.get('nome_equipamento', 'Sem Nome'),
        float(request.form.get('potencia') or 0),
...
def requisicoes():
    ...
    ''').fetchall()
    conn.close()
    return render_template('requisicoes.html', requisicoes=reqs, estoque_itens=itens_estoque)

@app.route('/salvar_requisicao', methods=['POST'])
def salvar_requisicao():
    conn = get_db_connection()
    conn.execute('''
        INSERT INTO requisicoes_compras (equipamento_tipo, especificacao_desejada, quantidade) 
        VALUES (?, ?, ?)
    ''', (
        request.form.get('equipamento_tipo', ''),
        request.form.get('especificacao_desejada', ''),
...
def cotar_internet(id):
    ...
    if req:
        ...
        conn.execute('''
            UPDATE requisicoes_compras 
            SET preco_cotado=?, potencia_cotada=?, depreciacao_sugerida=?, status="Cotado - Aguardando Confirmação" 
            WHERE id=?
        ''', (preco, pot, dep, id))
        conn.commit()
    conn.close()
...
def efetivar_compra(id):
    ...
    if req:
        ...
        conn.execute('''
            INSERT INTO maquinas (nome_equipamento, potencia, consumo_eletrico, velocidade, avanco, comprimento_max, diametro_max, frequencia_manutencao, horas_trabalhadas, preco_compra, depreciacao_mensal, valor_venda_final, custo_minuto_maquina) 
            VALUES (?, ?, ?, "3000", "15000", 500, 300, 1000, 0, ?, ?, ?, ?)
        ''', (
            f"{req['equipamento_tipo']} - {req['especificacao_desejada']}", 
            pot, pot * 0.7, preco, dep, preco * 0.2, c_mm
        ))
...
def materiais():
    ...
    ''').fetchall()
    conn.close()
    return render_template('materiais.html', materiais=mats, estoque_itens=itens_estoque)

@app.route('/salvar_material', methods=['POST'])
def salvar_material():
    codigo = request.form.get('codigo_material', '')
    nome = request.form.get('nome_material', 'Sem Nome')
    preco = float(request.form.get('preco_unidade') or 0)
    dimensoes = request.form.get('dimensoes', '')
    volume = float(request.form.get('volume_disponivel') or 0)
    ...
    try:
        ...
        return "Erro Pedagógico: Código de material duplicado!"
        
    # CORRIGIDO: Modificado de 'materials' para 'materiais' em português
...
def engenharia():
    ...
    conn.close()
    return render_template('engenharia.html', produtos=prods, maquinas=maqs, materiais=mats, composicoes=comps, estoque_itens=itens_estoque)
...
def precificacao():
    ...
    conn.close()
    return render_template('precificacao.html', produtos=prods, precos_salvos=salvos, estoque_itens=itens_estoque)

@app.route('/salvar_preco', methods=['POST'])
def salvar_preco():
    conn = get_db_connection()
    conn.execute('''
        INSERT OR REPLACE INTO formacao_precos (produto_id, imposto_municipal, imposto_estadual, imposto_federal, margem_lucro, preco_venda_final) 
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (
        int(request.form.get('produto_id', 0)), 
        float(request.form.get('imposto_municipal') or 0), 
        float(request.form.get('imposto_estadual') or 0), 
        float(request.form.get('imposto_federal') or 0), 
        float(request.form.get('margem_lucro') or 0), 
        float(request.form.get('preco_venda_final') or 0)
    ))
    conn.commit()
    conn.close()
...
def vendas():
    conn = get_db_connection()
    prods = conn.execute('''
        SELECT p.id, p.codigo_produto, p.nome_produto, COALESCE(fp.preco_venda_final, 0) AS preco_venda_final, 
               COALESCE(e.quantidade_disponivel, 0) AS estoque_atual 
        FROM produtos p 
        LEFT JOIN formacao_precos fp ON p.id = fp.produto_id 
        LEFT JOIN estoque_produtos e ON p.id = e.produto_id
    ''').fetchall()
    
    peds = conn.execute('''
        SELECT pv.*, p.codigo_produto, p.nome_produto, 
               COALESCE(fp.preco_venda_final, 0) AS preco_venda_final, 
               COALESCE(fp.imposto_municipal, 0) AS imposto_municipal, 
               COALESCE(fp.imposto_estadual, 0) AS imposto_estadual, 
               COALESCE(fp.imposto_federal, 0) AS imposto_federal 
    ...
    conn.close()
    return render_template('vendas.html', produtos=prods, pedidos=peds, estoque_itens=itens_estoque)
...
def abastecer_estoque():
    prod_id = int(request.form.get('produto_id', 0))
...
def lancar_venda():
    ...
    conn.execute('''
        ...
        VALUES (?, ?, ?, ?)
    ''', (prod_id, qtd, float(request.form.get('desconto_percentual') or 0), request.form.get('observacoes', '')))
    conn.commit()
    conn.close()
...
def imprimir_nf(pedido_id):
    ...
    v_desc = sub * ((ped['desconto_percentual'] or 0) / 100)
    liq = sub - v_desc
    
    return render_template('nota_fiscal.html', 
                           p=ped, subtotal=sub, v_desconto=v_desc, total_liquido=liq, 
                           v_municipal=liq*(ped['imposto_municipal']/100), 
                           v_estadual=liq*(ped['imposto_estadual']/100), 
                           v_federal=liq*(ped['imposto_federal']/100), 
